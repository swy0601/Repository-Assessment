...
#         super().__init__()
#         self.gtn=_GTN(num_edge, num_channels, w_in, w_out, num_layers, norm)
#         self.g=None
#
#     def summon_graph(self,g):
#         if self.g==g:
#             ret
#
#
...
#         else:
#             H = H * ((torch.eye(H.shape[0]) == 0).type(torch.FloatTensor)).to(H.device) + torch.eye(H.shape[0]).type(
#                 torch.FloatTensor).to(H.device)
